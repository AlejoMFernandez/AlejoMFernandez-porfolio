    ...
    const runShootingStar = () => {
      gsap.fromTo(shootingStar,
        { x: 0, y: 0, opacity: 0 },
        {
          ...
          duration: 0.75,
          ease: 'none',
          onComplete: () => { gsap.delayedCall(6 + Math.random() * 5, runShootingStar) }
        }
      )
    }
...
<template>
  <div ref="rootRef" class="tile-cover">
    <svg viewBox="30 150 740 740" preserveAspectRatio="xMidYMid slice" class="tile-svg"
         role="img" aria-label="Refugio del Cóndor mountain lodge">
      <defs>
        <linearGradient id="patt-sky" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#050810"/>
          <stop offset="45%"  stop-color="#0C1422"/>
          <stop offset="100%" stop-color="#14202E"/>
        </linearGradient>
        <radialGradient id="patt-ember-glow" cx="0.34" cy="0.74" r="0.38">
          <stop offset="0%"   stop-color="#E08D3C" stop-opacity="0.20"/>
          <stop offset="100%" stop-color="#E08D3C" stop-opacity="0"/>
        </radialGradient>
        <radialGradient id="patt-vignette" cx="0.5" cy="0.5" r="0.72">
          <stop offset="55%"  stop-color="#000000" stop-opacity="0"/>
          <stop offset="100%" stop-color="#000000" stop-opacity="0.55"/>
        </radialGradient>
        <filter id="patt-blur-stars"><feGaussianBlur stdDeviation="0.6"/></filter>
        <filter id="patt-blur-smoke"><feGaussianBlur stdDeviation="2"/></filter>
      </defs>

      <rect width="800" height="1000" fill="url(#patt-sky)"/>

      <!-- Stars -->
      <g filter="url(#patt-blur-stars)">
      ...
      <!-- Shooting star -->
      <g class="pat-shooting-star" transform="translate(180 80)" opacity="0" filter="url(#patt-blur-stars)">
      ...
      <g transform="translate(268, 616)">
        ...
        <!-- Humo (animado) -->
        <g class="pat-smoke" filter="url(#patt-blur-smoke)" opacity="0.28">
      ...
      <!-- Ember glow (animado) + vignette -->
      <rect class="pat-ember" width="800" height="1000" fill="url(#patt-ember-glow)" opacity="0.55"/>
      <rect width="800" height="1000" fill="url(#patt-vignette)"/>
    </svg>
  </div>
</template>
