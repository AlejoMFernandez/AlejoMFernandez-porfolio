...
onMounted(() => {
  ...
  // ── Botella: flotación vertical suave
  if (bottle) {
  ...
  // ── Hoja izquierda grande: balanceo lento (brisa suave)
  if (leafL) {
    tweens.push(gsap.fromTo(leafL,
      { rotation: -3, transformOrigin: '100% 90%' },
      { rotation:  2, transformOrigin: '100% 90%',
        duration: 4.0, ease: 'sine.inOut', yoyo: true, repeat: -1 }
    ))
  }

  // ── Hoja derecha grande: balanceo opuesto
  if (leafR) {
    tweens.push(gsap.fromTo(leafR,
      { rotation:  2.5, transformOrigin: '0% 90%' },
      { rotation: -2, transformOrigin: '0% 90%',
        duration: 4.4, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 0.5 }
    ))
  }
  ...
  if (leafBotL) {
    tweens.push(gsap.fromTo(leafBotL,
      ...
      { rotation:  2, transformOrigin: '80% 80%',
        duration: 5.2, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 1.0 }
    ))
  }
  ...
  if (leafBotR) {
    tweens.push(gsap.fromTo(leafBotR,
      ...
      { rotation: -2, transformOrigin: '20% 80%',
        duration: 5.2, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: 1.4 }
    ))
  }

  // ── Tallo central: micro balanceo
  if (stem) {
    tweens.push(gsap.fromTo(stem,
      { rotation: -1, transformOrigin: '50% 100%' },
      { rotation:  1, transformOrigin: '50% 100%',
        duration: 4.6, ease: 'sine.inOut', yoyo: true, repeat: -1 }
    ))
  }

  // ── Pétalos: deriva suave individual
  petals.forEach((p, i) => {
    ...
    tweens.push(gsap.fromTo(p,
      { x: 0, y: 0 },
      { x: dx, y: dy,
        duration: 2.8 + i * 0.6, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: i * 0.4 }
    ))
  })

  // ── Puntitos: deriva flotante
  dots.forEach((d, i) => {
  ...
  // ── Círculo decorativo externo: pulso de opacidad
  if (ringOuter) {
...
<template>
  <div ref="rootRef" class="pra-cover">
    <svg viewBox="0 0 1600 1000" preserveAspectRatio="xMidYMid slice" class="pra-svg"
         role="img" aria-label="Puraa natural cosmetics e-commerce preview">
      <defs>
        <!-- Fondo oscuro unificado: verde profundo -->
        <linearGradient id="pra-bg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%"   stop-color="#1e3d28"/>
          <stop offset="55%"  stop-color="#152c1e"/>
          <stop offset="100%" stop-color="#0c1c12"/>
        </linearGradient>
        <!-- Halo central suave -->
        <radialGradient id="pra-halo" cx="0.28" cy="0.52" r="0.45">
          <stop offset="0%"   stop-color="#4a8c5c" stop-opacity="0.18"/>
          <stop offset="100%" stop-color="#4a8c5c" stop-opacity="0"/>
        </radialGradient>
        <!-- Vignette -->
        <linearGradient id="pra-vignette" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#000000" stop-opacity="0.28"/>
          <stop offset="50%"  stop-color="#000000" stop-opacity="0"/>
          <stop offset="100%" stop-color="#000000" stop-opacity="0.32"/>
        </linearGradient>
        <!-- Botella cuerpo: crema translúcido que resalta sobre oscuro -->
        <linearGradient id="pra-bottle-body" x1="0" y1="0" x2="1" y2="0">
          <stop offset="0%"   stop-color="#d8c9a8"/>
          <stop offset="50%"  stop-color="#f0e8d4"/>
          <stop offset="100%" stop-color="#c4b490"/>
        </linearGradient>
        <!-- Líquido serum -->
        <linearGradient id="pra-liquid" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%"   stop-color="#6dbf82" stop-opacity="0.90"/>
          <stop offset="100%" stop-color="#3a7a4d" stop-opacity="0.98"/>
        </linearGradient>
      </defs>

      <!-- Fondo unificado verde oscuro -->
      <rect width="1600" height="1000" fill="url(#pra-bg)"/>
      <rect width="1600" height="1000" fill="url(#pra-halo)"/>
      <rect width="1600" height="1000" fill="url(#pra-vignette)"/>

      <!-- ── IZQUIERDA: composición botánica ── -->
      <g transform="translate(50 60)">
        ...
        <g class="pra-bottle" transform="translate(195 175) scale(2.6)">
          ...
          <rect x="51" y="22"  width="14" height="10" fill="url(#pra-bottle-body)" stroke="#a08868" stroke-width="0.8"/>
          <!-- Hombro + cuerpo -->
          <path d="M 28 32 L 88 32 L 94 52 L 94 222 Q 94 244 56 244 Q 18 244 18 222 L 18 52 Z"
                fill="url(#pra-bottle-body)" stroke="#a08868" stroke-width="1.4"/>
          <!-- Líquido serum -->
          ...
          <text x="56" y="116" text-anchor="middle"
                font-family="'Inter',sans-serif" font-weight="700" font-size="10.5"
                fill="#162a1a" letter-spacing="2.5">PURAA</text>
          <line x1="34" y1="121" x2="78" y2="121" stroke="#5aad40" stroke-width="0.6"/>
          <text x="56" y="138" text-anchor="middle"
                font-family="'Inter',sans-serif" font-size="6.5" fill="#2a5c1a">SERUM FACIAL</text>
          <text x="56" y="152" text-anchor="middle"
                font-family="'Inter',sans-serif" font-size="5.8" fill="#3a6a2a" opacity="0.7">30 ml · ORGANIC</text>
      ...
      <!-- ── PANEL DERECHO ── -->
      <text x="1520" y="455" text-anchor="end"
            font-family="'Inter','Helvetica Neue',sans-serif"
            font-weight="900" font-size="215"
            letter-spacing="-8" fill="rgba(244,237,216,0.92)">PURAA</text>

      <circle cx="1520" cy="418" r="16" fill="#7dd88a"/>
      <line x1="840" y1="510" x2="1520" y2="510" stroke="#7dd88a" stroke-width="1.5" opacity="0.45"/>

      <text x="1520" y="548" text-anchor="end"
            font-family="'Inter',sans-serif" font-style="italic" font-weight="400"
            font-size="22" letter-spacing="5" fill="#9de8a0">
        bioconciencia · alquimia vegetal
      </text>

      <!-- Decoración derecha: pequeños puntos a media altura -->
      <g fill="#7dd88a" opacity="0.35">
        <circle cx="860" cy="600" r="3"/>
        <circle cx="880" cy="640" r="2"/>
        <circle cx="855" cy="680" r="3"/>
        <circle cx="878" cy="720" r="2"/>
        <circle cx="858" cy="760" r="3"/>
      </g>

      <!-- Corner brackets -->
      <g stroke="#8db580" stroke-width="1.5" fill="none" opacity="0.45">
...
<style scoped>
.pra-cover { position: absolute; inset: 0; width: 100%; height: 100%; overflow: hidden; }
.pra-svg   { width: 100%; height: 100%; display: block; }
</style>
